...
class InvestForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, int $plan_id = 0) {
    ...
    if ($this->investmentService->getActiveInvestment((int) $this->currentUser->id())) {
      $this->messenger()->addWarning($this->t('You already have an active FXC investment.'));
    ...
    $form['summary'] = [
      '#markup' => '<div class="invest-summary">'
        ...
        . '<p class="invest-note">' . $this->t('AI profit accrues daily and is credited to your wallet every Saturday morning.') . '</p>'
        . '</div>',
    ...
    $form['confirm'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('I understand that ₹@amount will be debited from my wallet and invested in the FXC Global model.', [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    try {
      ...
      $this->messenger()->addStatus($this->t('Your FXC investment is now active. AI profit will accrue through the week.'));
      $form_state->setRedirect('niftybot_investment.dashboard');
    }
